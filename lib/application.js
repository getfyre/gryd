...
var Application = module.exports = function (name, path, gryd, global) {
    var level = (gryd.env === 'dev') ? 'debug' : 'info';
    this.log = Bunyan.createLogger({name: name + "." + gryd.env, stream: formatOut, level: level});
    this.name = name;
    this.path = path;
    this.gryd = gryd;
    this.global = global;
...
Application.prototype.connectDatabase = function (callback) {
    ...
    if (self.config.db) {
        ...
        var db = mongoose.connect(self.config.db, opts, function () {
            self.log.debug('mongoose connect');
